    ...
    <div className="slot-container">
      ...
      <div className={`slot-machine ${inBonus && bonusMode === 'playing' ? 'bonus-active' : ''}`}>
        ...
        <div className="slot-grid" style={inBonus && bonusMode === 'playing' ? { marginTop: '80px' } : {}}>
          ...
          {inBonus && !currentBonusSpin && Array.from({ length: GRID_ROWS * GRID_COLS }, (_, i) => (
            <div key={i} className="slot-cell bonus-cell-empty"><span className="symbol-emoji">·</span></div>
          ))}
        </div>
      </div>
        ...
        <div className={`win-display ${result.rescued ? 'rescue' : result.isWin ? 'win' : 'lose'}`}>
          {result.error ? (
            ...
            <>
              ...
              {result.bonusTriggered && <div className="rescue-message">🎰 ¡SCATTER BONUS activado!</div>}
            </>
          ) : (
